...
public class FunctionFactory
{
    ...
    public void AddFunction(MethodInfo method)
    {
        ...
        if (attributes.Length > 0)
        {
            ...
            var tool = new OnnxGenAIChatFunctionTool() {
                Name = method.Name,
                Description = funcDescription ?? $"The {method.Name} function",
                Parameters = json
            };
            _functions.TryAdd(method, tool);
        }
    }

    public IEnumerable<OnnxGenAIChatFunctionTool> GetChatTools()
    {
    ...
    private static string? CallFunction(MethodInfo methodInfo, OnnxGenAIChatFunctionTool chatTool, string argumentsAsJson)
    {
    ...
    private Dictionary<MethodInfo, OnnxGenAIChatFunctionTool> _functions = new();
}